# Erzeugt die VGR-Daten für alle Übungen.
# Ausführen aus dem Projektordner:  Rscript data/_generate_data.R
#
# KANONISCHES SCHEMA (alle Übungen/Slides nutzen genau diese Spalten):
#   bws_lang  (tidy/long): jahr<int>, quartal<int 1..4>, bereich_code<chr>,
#                          bereich_name<chr>, wert<num, Mio. EUR, jeweilige Preise>
#   Wirtschaftsbereiche (10, VGR-Aggregation, NACE-ähnlich):
...
#     OQ  Öffentliche Dienstleister, Erziehung, Gesundheit
...
#   lieferung_pdf.pdf     - kleine Tabelle als PDF (für pdftools)
...
bereiche <- tribble(
  ...
  "L",  "Grundstücks- und Wohnungswesen",                              78000,
  "MN", "Unternehmensdienstleister",                                     95000,
  "OQ", "Öffentliche Dienstleister, Erziehung, Gesundheit",            115000,
...
# Vintage 2: 2023/2024 leicht revidiert, ein neues Quartal 2025Q1 ergänzt
revidiert <- bws_lang |>
...
korresp <- tribble(
  ...
  "L",  "L",  "Grundstücks- und Wohnungswesen",                   1,
  "MN", "MN", "Unternehmensdienstleister",                         1,
  "OQ", "OQ", "Öffentliche Dienstleister, Erziehung, Gesundheit", 1,
  "RU", "RU", "Sonstige Dienstleister",                            1
)
write_csv(korresp, "data/klassifikation_korrespondenz.csv")

# Excel-Lieferung: realistische "Behörden-Formatierung" mit Titelzeilen über den Daten
if (requireNamespace("openxlsx", quietly = TRUE)) {
  ...
  writeData(wb, "Metadaten", c(
    "VGR-Datensatz",
    "Bruttowertschöpfung in jeweiligen Preisen, Mio. EUR",
    "Datenstand: 2024Q4"
  ))
  addWorksheet(wb, "Daten")
  writeData(wb, "Daten", "Bruttowertschöpfung nach Wirtschaftsbereichen (Mio. EUR)",
            startRow = 1, startCol = 1)
  writeData(wb, "Daten", "Datenstand 2024Q4", startRow = 2, startCol = 1)
  # Wide-Format wie in Excel üblich: Kopfzeile 4, Daten ab Zeile 5
  wide <- bws_lang |>
...
# PDF-Tabelle (eine Seite) ohne Zusatzpakete: Cairo-PDF + grid.
# Cairo bettet die Schrift ein, damit Umlaute und ß in allen PDF-Viewern
# zuverlässig dargestellt werden.
local({
  tab <- bws_lang |>
    filter(jahr == 2024, quartal == 4) |>
    transmute(Bereich = bereich_code, Bezeichnung = bereich_name,
              `Wert (Mio. EUR)` = format(wert, big.mark = ".", decimal.mark = ","))
  grDevices::cairo_pdf("data/lieferung_pdf.pdf", width = 8.3, height = 5.8,
                       family = "sans")
  grid::grid.newpage()
  grid::grid.text("Bruttowertschöpfung 2024Q4, Mio. EUR",
...
if (requireNamespace("DBI", quietly = TRUE) && requireNamespace("RSQLite", quietly = TRUE)) {
  ...
  message("RSQLite fehlt - edo_demo.sqlite übersprungen (in install_packages.R enthalten).")
}
