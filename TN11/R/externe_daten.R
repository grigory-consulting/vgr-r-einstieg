...
summe_bereich <- daten %>% 
  ...
  summarise(summe_2024 = sum(wert)) %>% # Aggregation -> neuer Datensatz und kleiner als ursprünglich
  arrange (desc(summe_2024))

# mit write.xlsx
# write.xlsx(summe_bereich, "output/ergebnis_summe.xlsx")

# openxlsx

wb <- createWorkbook() # leere Excel-Datei
